...
re2c:define:YYGETCONDITION = "*ScanStateArrayPeek(&scanner.condition)";
re2c:define:YYGETCONDITION:naked = 1;
re2c:define:YYSETCONDITION = "{enum ScanState *n = ScanStateArrayNew(&scanner.condition); *n = scanner.condition = @@;}";
re2c:define:YYSETCONDITION:naked = 1;
*/

#define ARRAY_NAME ScanState
#define ARRAY_TYPE enum ScanState
#define ARRAY_STACK
...
struct Scanner {
	...
	struct ScanStateArray condition;
	enum Symbol symbol;
...
void Scanner_(void) {
	...
	ScanStateArray_(&scanner.condition); /* Generated by `re2c`. */
	scanner.symbol = END;
...
int Scanner(void) {
	...
	do { /* Try: read all contents from `stdin` at once. */
		...
		if(!(condition = ScanStateArrayNew(&scanner.condition))) break;
		*condition = yyccode;
...
/*!re2c
	...
	<comment, string, character, macro> * { goto reset; }
	// http://re2c.org/examples/example_03.html
	<code> "\x00" { if(scanner.limit - scanner.cursor <= YYMAXFILL) return END;
		goto reset; }
	<doc, param, math, comment, string, character, macro> "\x00"
	...
	<doc> newline { scanner.line++; return NEWLINE; }
	<param, math, code, comment> newline { scanner.line++; goto reset; }
	<string, character> newline { return errno = EILSEQ, END; }
	<macro> newline { scanner.line++; return pop_call(); }
	// Continuation.
	<string, character, macro> "\\" newline { scanner.line++; goto scan; }
	<doc> art / [^/] { scanner.line++; return NEWLINE; }
	<param, math> art / [^/] { scanner.line++; goto reset; }

	<doc> "*""/" { return pop_call(); }
	<param, math> "*""/" { return errno = EILSEQ, END; }
	...
	<doc> "`" { return push(DOC_MATH) ? BEGIN_MATH : END; }
	<math> "`" { return pop() ? END_MATH : END; }
	<doc> "{" { return push(DOC_PARAM) ? DOC_LBRACE : END; }
	<param> "{" { return errno = EILSEQ, END; }
	<doc> "}" { return DOC_RBRACE; }
	<param> "}" { return pop() ? DOC_RBRACE : END; }
	...
	<code, macro> doc / [^/] { return push(DOC) ? BEGIN_DOC : END; }
	<code, marco> begin_comment { return push_call(COMMENT); }
	<code> macro { return push_call(MACRO); }
	<code> "L"? "\"" { return push_call(STRING); }
	<code> "'" { return push_call(CHAR); }
	...
	<comment> "\\". { goto scan; } // is this really necessary?
	<comment> end_comment { return pop_call(); }

	<string> "\"" { return pop() ? CONSTANT : END; }
	<character> "'" { return pop() ? CONSTANT : END; }
