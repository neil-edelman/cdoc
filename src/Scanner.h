...
#define SYMBOL(X) \
	...
	X(CONSTANT, OUT_C), X(ID, OUT_C), X(ID_GENERIC, OUT_C), \
	X(ID_GENERIC_TWO, OUT_C), X(ID_GENERIC_THREE, OUT_C), X(STRUCT, OUT_C), \
	X(UNION, OUT_C), X(ENUM, OUT_C), X(TYPEDEF, OUT_C), \
	...
	X(ESCAPED_EACH, OUT_DOC), \
	X(NEWLINE,	OUT_DOC), X(WORD, OUT_DOC), \
